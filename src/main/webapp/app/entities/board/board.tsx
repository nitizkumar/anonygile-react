...
export const Board = (props: IBoardProps) => {
  ...
  return (
    <div>
      <h2 id="board-heading">
        <Translate contentKey="anonygileApp.board.home.title">Boards</Translate>
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          <FontAwesomeIcon icon="plus" />
          &nbsp;
          <Translate contentKey="anonygileApp.board.home.createLabel">Create new Board</Translate>
        </Link>
      </h2>
        ...
        {boardList && boardList.length > 0 ? (
            ...
            <tbody>
              {boardList.map((board, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{board.name}</td>
                  <td>{board.type}</td>
                  <td>{board.owner ? board.owner.id : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="anonygileApp.board.home.notFound">No Boards found</Translate>
            </div>
          )
        )}
      </div>
